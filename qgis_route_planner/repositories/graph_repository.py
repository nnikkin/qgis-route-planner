...
class RoadGraphRepository:
    ...
    def __insert_edges_from_mapping(
        ...
        bridge_expr = self.__build_bridge_expr(other_col)
        lanes_expr = self.__build_tag_int_expr(other_col, "lanes")
            ...
            self.__db.execute_nonquery("""
                ...
                SET
                    ...
                    avg_speed_estimated = LEAST(
                        COALESCE(e.avg_speed_estimated, s.max_speed_kmh * 0.8),
                        s.max_speed_kmh * 0.8
                    ),
    ...
    def create_restrictions_from_pipes(
        ...
        for layer in pipe_layers:
            ...
            geom_col = table_mapping.get(ColumnRole.GEOMETRY) or "geom"
            name_col = table_mapping.get(ColumnRole.NAME)
            man_made_col = table_mapping.get(ColumnRole.MAN_MADE) or self.__existing_column(layer.name, "man_made")
            other_col = table_mapping.get(ColumnRole.OTHER)
            ...
            pipeline_condition = self.__tag_equals_condition("p", man_made_col, other_col, "man_made", "pipeline")
            overground_condition = sql.SQL("""
                (
                    COALESCE((p.{other_col}::hstore)->'location', (p.{other_col}::hstore)->'localtion') = 'overground'
                )
            """).format(other_col=sql.Identifier(other_col)) if other_col else sql.SQL("FALSE")
            ...
            insert_query = sql.SQL("""
                ...
                SELECT
                    ...
                    'height=' || pipe_data.max_height::text || ';width=0;weight=0' AS value_text,
    ...
    def create_parking_areas(
        ...
        for layer in parking_layers:
            ...
            amenity_condition = self.__tag_equals_condition(
                "p",
                amenity_col,
                other_col,
                "amenity",
        ...
        try:
            self.__db.execute_nonquery("""
                ...
                SELECT
                    ...
                    'height=' || COALESCE(e.max_height, 0)::text ||
                    ';width=' || COALESCE(e.max_width_m, 0)::text ||
                    ';weight=' || COALESCE(e.max_weight_t, 0)::text AS value_text,
    ...
    def __pgr_with_points(
        ...
        return self.__db.execute_query(
            ...
            ORDER BY r.seq
            """,
            self.__routing_edges_sql(profile, restriction_nodes, route_speed_kmh),
            point_rows_sql,
            -start_pid,
            -end_pid,
        )
    ...
    def get_routes_with_points(
        ...
        for index, point in enumerate(route_points, start=1):
            ...
            point_values.append(f"({pid}, {int(point.edge_id)}, {fraction}, 'b')")
    ...
    def __route_cost_expr(self, default_column: str, route_speed_kmh: float | None) -> str:
        if route_speed_kmh is None or route_speed_kmh <= 0:
            return default_column
        return (
            f"CASE WHEN {default_column} < 0 THEN -1 "
            f"ELSE length_m / ({float(route_speed_kmh)} / 3.6) END"
        )
    ...
    def __get_line_table(self, layers: list[Layer]) -> str | None:
        for layer in layers:
            if layer.role is LayerRole.ROADS and layer.geom_type is GeometryType.LINESTRING:
                return layer.name
        for layer in layers:
    ...
    def __get_point_table(self, layers: list[Layer]) -> Layer | None:
        for layer in layers:
            if layer.role is LayerRole.POINTS:
                return layer
        for layer in layers:
            if layer.geom_type is GeometryType.POINT:
                return layer
        return None
    ...
    def __build_bridge_expr(self, other_col: str | None):
        if not other_col:
            return sql.SQL("FALSE")
        return sql.SQL("""
            CASE
                WHEN (l.{}::hstore)->'bridge' IN ('yes', 'true', '1', 'viaduct') THEN TRUE
                ELSE FALSE
            END
        """).format(sql.Identifier(other_col))
    ...
    def __build_tag_number_expr(
        ...
        return sql.SQL("""
            NULLIF(
                replace(substring(({}.{})::hstore->{} from '[0-9]+[,.]?[0-9]*'), ',', '.'),
                ''
            )::double precision
        """).format(
